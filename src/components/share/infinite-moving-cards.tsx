  ...
  return (
    ...
    >
      ...
      >
        ...
        {(topShops?.data?.data && topShops.status === 'success') && topShops?.data?.data.filter((shop) => shop._id !== (user?.sellerId as Seller)?._id).map((item, idx) => (
          <Card key={item._id} className="w-[350px] overflow-hidden">
            <div className="relative">
              ...
              />)}
              {!item.img_cover && (<div className="w-full h-28 bg-slate-700" />)}
              <div onClick={() => handleClickViewDetailShop(item.slug)} className="absolute  -bottom-6 cursor-pointer left-4">
            ...
            <CardContent className="pt-12 pb-4">
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{item.city.split('-')[1]}</p>
                </div>
                <Badge variant="secondary" className="flex items-center gap-1">
                  <Users className="w-3 h-3" />
                  {(item.followers.length ?? 0).toLocaleString() + ' Quan tâm'}
                </Badge>

              </div>
              <div className="flex items-center gap-1">
                <Star className="w-4 h-4 fill-primary text-primary" />
                <span className="font-medium">{(item.averageRating ?? 0)}</span>
              </div>
            </CardContent>
          </Card>
        ))}
