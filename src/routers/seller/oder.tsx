...
export const OderRoute = () => {
  ...
  const handleClickEditDetail = (order: oderDetail) => {
    for (const key in order.status_oder) {
      const element = order.status_oder[
        key as keyof typeof order.status_oder
      ] as { status: boolean; created_at?: string }
      if (element.status) {
        if (key === "canceled") {
          setCurrentStep(
            stepsCancels.map((step) => ({
              ...step,
              time: formatDate(
                order?.status_oder[
          ...
          const stepId = stepCoppy
            .reverse()
            .find((step) => step.status === key)?.id
          if (stepId) {
            setCurrentStep((prev) =>
              prev.map((step) =>
                step.id <= stepId
  ...
  const handleUpdateStatusOrder = (
    status: "success" | "canceled" | "shipping"
  ) => {
  ...
  return (
    <>
      ...
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead className="cursor-pointer">Mã đơn hàng</TableHead>
            <TableHead className="cursor-pointer">Khách hàng</TableHead>
            <TableHead className="cursor-pointer">Thời gian đặt hàng</TableHead>
            ...
            orders?.data?.data?.map((order) => (
              <TableRow key={order?._id}>
                ...
                <TableCell className="capitalize font-semibold">
                  {order?.oder_id?.user_id?.username}
                </TableCell>
                <TableCell>
                  {formatDate(order?.status_oder?.pending.created_at)}
                </TableCell>
                ...
                <TableCell className="text-right">
                  <Button
                    onClick={() => handleClickEditDetail(order)}
                    variant="outline"
                    size="icon"
                  >
        ...
        >
          ...
          <div className="grid gap-2">
                    ...
                    {currentStep?.map((step, index) => (
                      ...
                      >
                        <div className="flex flex-col items-center flex-grow ">
                          <div
                            className={`flex items-center justify-center p-2 rounded-full border-[3px] ${step.preview === "hidden" ? "border-[#9ca3af]" : "border-[#4ade80]"}`}
                          >
                            <step.Icon
                              color={`${step.preview === "hidden" ? "#9ca3af" : "#4ade80"}`}
                            />
                          </div>
                          <div className="mt-4 text-center">
                            ...
                            <time className="text-xs text-nowrap font-normal">
                              {step?.time || ""}
                            </time>
                          </div>
                        </div>
                  ...
                  <>
                    ...
                    >
                      {currentStep?.map((step, index) => (
                        ...
                        >
                          <div className="flex flex-col items-center flex-grow ">
                            <div
                              className={`flex items-center justify-center p-2 rounded-full border-[3px] ${step.preview === "hidden" ? "border-[#9ca3af]" : "border-[#f87171]"}`}
                            >
                              <step.Icon
                                color={`${step.preview === "hidden" ? "#9ca3af" : "#f87171"}`}
                              />
                            </div>
                            <div className="mt-4 text-center">
                              <div className="text-sm font-medium text-nowrap">
                    ...
                    <div className="flex items-center justify-between font-light my-[2px]">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-destructive">
                          {oderDetail?.status_oder?.canceled?.message}
                        </span>
                      </div>
                    </div>
                  </>
                ...
                <div className="flex items-center justify-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="capitalize block">
                      {oderDetail?.color?.name} (x{oderDetail?.quantity})
                    </span>
                  </div>
                </div>
                <span>
                  {formatNumberToVND(
                    (oderDetail?.price ?? 0) * (oderDetail?.quantity ?? 0)
                  )}
                </span>
              </div>
            ...
            <div>
              ...
              <div className="capitalize flex items-center gap-2">
                <p className="font-medium">
                  {" "}
                  {oderDetail?.oder_id?.type_pay == "cash"
            ...
            <div>
              ...
              <div>
                ...
                <p className="capitalize">
                  {oderDetail?.oder_id?.address_id?.address + " ,"}
                </p>
                <p>
          ...
          {oderDetail?.status_oder?.pending?.status ? (
            <div className="flex items-center gap-5">
              <Button
                className="bg-yellow-500 hover:bg-yellow-600 hover:opacity-90"
              ...
              <Button
                variant="destructive"
                onClick={() => handleUpdateStatusOrder("canceled")}
              >
                Hủy đơn hàng
              </Button>
            </div>
            ...
            <Button
              ...
              onClick={() => handleUpdateStatusOrder("success")}
            >
              Xác nhận hoàn tất
            </Button>
          ) : oderDetail?.status_oder?.success?.status ? (
      ...
      <Dialog open={openListReason} onOpenChange={setOpenListReason}>
        <DialogContent>
          ...
          <div>
            ...
            <Button
              onClick={() => setOpenListReason(false)}
      ...
      <AlertDialog
        open={["shipping", "success", "canceled"].includes(
          statusUpdate?.status!
        )}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {statusUpdate?.status === "shipping"
                ...
                : statusUpdate?.status === "success"
                  ? "Bạn có chắc chắn muốn xác nhận giao hàng không?"
                  : "Bạn có chắc chắn muốn hủy đơn hàng này không"}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
              onClick={() => setStatusUpdate({ status: null, data: null })}
            >
              Hủy
            </AlertDialogCancel>
