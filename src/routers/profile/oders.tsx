...
const listReasons = [
  ...
  { id: 3, title: "Thời gian giao hàng quá lâu", status: false },
  { id: 4, title: "Chi phí vận chuyển cao", status: false },
  { id: 5, title: "Sản phẩm không còn cần thiết", status: false },
  { id: 6, title: "Đặt nhầm sản phẩm", status: false },
  { id: 7, title: "Không hài lòng với dịch vụ", status: false },
...
export const OdersRoute = () => {
  ...
  const [oderDetailId, setOderDetailId] = useState<string>()
  ...
  const handleClickCancelOrders = (orderDetailId: string) => {
    setOpenListReason(true)
    setOderDetailId(orderDetailId)
  }

  useEffect(() => {
    if (!openListReason) {
      setOderDetailId(undefined)
    }
  }, [openListReason])
      ...
      <ProfileLayout>
        <Tabs defaultValue="all" className="w-full">
          <TabsList className="bg-transparent">
            <TabsTrigger
              className="bg-transparent data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none data-[state=active]:text-primary pb-4 px-8"
              value="all"
            >
              Tất cả
            </TabsTrigger>
          ...
          <div className="relative my-5 max-w-96">
                          ...
                          {orders?.data?.data &&
                            orders?.data?.data.map((order) => (
                              ...
                              >
                                  ...
                                  <div className="flex-grow">
                                    ...
                                    <div className="text-sm text-gray-500 line-clamp-2">
                                      <p className="line-clamp-1">
                                        ...
                                        {
                                          order?.address_id?.district.split(
                                            "-"
                                          )[1]
                                        }
                                        ,{" "}
                                        {order?.address_id?.city.split("-")[1]}
                                      </p>
                                    </div>
                                  </div>
                                </td>
                                ...
                                <td className="px-6 py-4">
                                  {formatNumberToVND(order?.totalPrice)}
                                </td>
                                <td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4">
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      handleOnclickViewDetail(order._id)
                                    }
                                  >
                                    Chi tiết
                                  </Button>
                  ...
                  <div className="flex items-center justify-between border-b-[1px] pb-5">
                    <div className="flex items-center gap-3 ">
                      <Avatar className="size-10 border">
                        <AvatarImage
                          src={oderDetail.sellerId.logo}
                          alt={oderDetail.sellerId.businessName}
                        />
                        <AvatarFallback>
                          {" "}
                    ...
                    <div className="basis-3/4 flex items-center gap-3">
                      <div className="size-20 min-w-20 max-w-20">
                        <img
                          ...
                          alt={oderDetail.product + oderDetail.color.name}
                        />
                      </div>
                      <div>
                        ...
                        <blockquote className="italic">
                          {"Màu: " +
                            oderDetail.color.name +
                            " - " +
                            oderDetail?.size?.name}
                        </blockquote>
                        <h3 className="font-semibold">
                          x{oderDetail.quantity}
                        </h3>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {oderDetail?.status_oder?.success?.status && (
                      <>
                        <div>
                          <p>
                            Đã giao thành công lúc:{" "}
                            {formatDate(
                              oderDetail?.status_oder?.success?.created_at
                            )}
                          </p>
                        </div>
                        <div className="flex items-center gap-3">
                      ...
                      <>
                        <div className="flex flex-col items-start gap-2">
                          <span className="capitalize">
                            đơn hàng đã được xác nhận vào lúc:{" "}
                            {formatDate(
                              oderDetail?.status_oder?.shipping?.created_at
                            )}
                          </span>
                        </div>
                        <div className="flex items-center gap-3">
                      ...
                      <>
                        <div>
                          <p className="text-[#57534e] capitalize">
                            đặt hàng lúc:{" "}
                            {formatDate(
                              oderDetail?.status_oder?.pending?.created_at
                            )}
                          </p>
                        </div>
                        <div className="flex items-center gap-3">
                        ...
                        <tbody>
                          {orders?.data?.data &&
                            orders?.data?.data.length === 0 && (
                              <div className="flex items-center p-4 pt-10 justify-center min-w-full">
                                <h2 className="scroll-m-20 border-b pb-2 text-2xl font-semibold tracking-tight first:mt-0">
                                  Hiện chưa có đơn hàng nào đã hoàn thành
                                </h2>
                              </div>
                            )}
                          {orders?.data?.data &&
